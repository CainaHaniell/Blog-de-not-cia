...
<link rel="shortcut icon" href="Img/Branco.png" type="image/x-icon">
    ...
    <title>Concurso - Vozes do Sertão</title>
</head>
<body>
    <header>
...
$dbname = "blog_noticias";
                        ...
                        // Consulta SQL para buscar notícias da categoria 'concurso'
                        $sql = "SELECT id, titulo, subtitulo, conteudo, fonte, caminho_imagem, data_publicacao FROM noticias WHERE categoria = 'concurso' ORDER BY data_publicacao DESC";
                        $result = $conn->query($sql);

                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                echo '<div class="noticia-card-com-foto">';
                                echo '    <span class="noticia-categoria">Concurso</span>';
