...
<head>
  ...
  <title>Emprego - Vozes do Sertão</title>
</head>
<body>
  <header>
      ...
      <section class="noticias-lista">
        ...
        <div id="container-noticias">
          <?php
            ...
            // Busca as notícias da categoria 'emprego' ordenadas pela data de publicação
            $sql = "SELECT id, titulo, subtitulo, conteudo, fonte, caminho_imagem, data_publicacao FROM noticias WHERE categoria = 'emprego' ORDER BY data_publicacao DESC";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                // Exibe as notícias em um loop
                while($row = $result->fetch_assoc()) {
                    echo '<div class="noticia-card-com-foto">';
                    echo '  <span class="noticia-categoria">Emprego</span>';
